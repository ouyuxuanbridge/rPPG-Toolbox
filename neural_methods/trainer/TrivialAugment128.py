...
# alltrivialaug2
min_max_vals = MinMaxVals(
    shear=MinMax(-0.1, 0.10),
    translate=MinMax(-10, 10),
    rotate=MinMax(-10, 10),
    enhancer=MinMax(0.5, 1.5),
...
def _shear_y_impl(pil_img, level):
  ...
  """
    level = float_parameter2(level,min_max_vals.shear.min, min_max_vals.shear.max)
...
def _translate_y_impl(pil_img, level):
  ...
  """
    level = int_parameter2(level,min_max_vals.translate.min, min_max_vals.translate.max)
    #if random.random() > 0.5:
        #level = -level
...
def _rotate_impl(pil_img, level):
    ...
    degrees = int_parameter2(level,min_max_vals.rotate.min, min_max_vals.rotate.max)
    #if random.random() > 0.5:
        #degrees = -degrees
...
randomerase = TransformT('Randomerase',
                         lambda img, l: RandomeraseDefault(img, int_parameter(l, img.size[
                             0] * min_max_vals.randomerase.max)))
