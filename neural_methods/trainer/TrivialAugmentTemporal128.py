...
def _gaussianlabel_impl(data_numpy, label_numpy, level):
    ...
    '''
    var = 0.5
    label_numpy = label_numpy + (var ** 0.5) * np.random.randn(4, 128)
...
def _labelwarping_impl(data_numpy, label_numpy, level):
    ...
    sigma = 1
    # print(sigma)
    label_numpy = magnitude_warp(label_numpy.reshape(1, 4*128, 1), sigma).reshape(4, 128)
...
def _lowfreqnoise_impl(data_numpy, label_numpy, level):
    magnitudescale = min_max_vals.lowfreqnoisemagnitude.max - float_parameter2(level,
                                                                               min_max_vals.lowfreqnoisemagnitude.min,
                                                                               min_max_vals.lowfreqnoisemagnitude.max)
    ...
    label_numpy = label_numpy + func.reshape(4, 128)
    return data_numpy, label_numpy
...
class TrivialAugmentTemporal128:
    def __init__(self):

        self.op1 = random.choices(POOL1, k=1)[0]
        
        self.op2 = random.choices(POOL2_T, k=1)[0]
                ...
                for sample_idx in range(N):
                    ...
                    for frame_idx in range(D):
                        if self.op1 == videonoise:
                            datapersample.append(
                                self.op1.pil_transformer(1., self.level1)(data_numpy[sample_idx, frame_idx, :, :, :]))
                        else:

                            #print(data_numpy.shape)
                            data_numpy_frame = data_numpy[sample_idx, frame_idx, :, :, :] * 255
                            data_numpy_frame = data_numpy_frame.astype(np.uint8)
                            #print(data_numpy_frame.shape)
                ...
                for sample_idx in range(N):
                    ...
                    for frame_idx in range(D):
                        if self.op2 == videonoise:
                            datapersample.append(
                                self.op2.pil_transformer(1., self.level2)(data_numpy[sample_idx, frame_idx, :, :, :]))
                        else:
